                ...
                <tbody>
                    {orders.map(order => (
                        <tr key={order.idpedido}>
                            ...
                            <td>
                                <button onClick={() =>
                                    handleViewOrder(order)}>Visualizar</button>
                                <button onClick={() => handleUpdateStatus(order.idpedido,
                                    '1')}>Em Preparo</button>
                                <button onClick={() => handleUpdateStatus(order.idpedido,
                                    '2')}>Pronto</button>
                                <button onClick={() => handleUpdateStatus(order.idpedido,
                                    '3')}>Entregue</button>
                                <button onClick={() =>
                                    handleDeleteOrder(order.idpedido)}>Excluir</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            <ModalContent>
                ...
                <div>
                    <p><strong>Data/Hora:</strong> {new
                        Date(order.hora).toLocaleString()}</p>
                ...
                <div className="products-section">
                    ...
                    {orderProducts.map((item) => (
                        <div key={item.idproduto_pedido} className="product-item">
                            <p><strong>{item.product?.nome}</strong> - R$
                                {item.product?.valor}</p>
...
const ProductSelectionModal = ({ onClose, onConfirm }) => {
    ...
    const handleProductSelect = (product) => {
        const existingProduct = selectedProducts.find(p => p.produtos_idproduto
            === product.idproduto);
        if (existingProduct) {
            setSelectedProducts(selectedProducts.filter(p => p.produtos_idproduto
                !== product.idproduto));
        } else {
            setSelectedProducts([
                ...selectedProducts,
                    ...
                    {products.map(product => (
                        <div key={product.idproduto} className="product-item">
                            <label>
                                <input
                                    type="checkbox"
                                    checked={selectedProducts.some(p => p.produtos_idproduto ===
                                        product.idproduto)}
                                    onChange={() => handleProductSelect(product)}
                                />
                                {product.nome} - R$ {product.valor}
                            </label>
                            {selectedProducts.some(p => p.produtos_idproduto ===
                                product.idproduto) && (
                                    <textarea
                                        placeholder="Observações"
                                        value={selectedProducts.find(p => p.produtos_idproduto ===
                                            product.idproduto)?.observacao || ''}
                                        onChange={(e) => handleObservationChange(product.idproduto,
                                            e.target.value)}
                                    />
                                )}
                        </div>
